        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Data Engine Failure Stratification: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study robot data engines. The submission-hardened claim is intentionally narrow: Select new robot data by uncovered failure mechanisms, not by task labels. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Data engine failure stratification should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.405 $\pm$ 0.011 & 0.446 $\pm$ 0.013 & 0.328 $\pm$ 0.015 \\
Data-augmented WAM & 0.492 $\pm$ 0.009 & 0.509 $\pm$ 0.018 & 0.407 $\pm$ 0.012 \\
Scalar uncertainty & 0.554 $\pm$ 0.012 & 0.580 $\pm$ 0.007 & 0.484 $\pm$ 0.018 \\
Ensemble disagreement & 0.595 $\pm$ 0.015 & 0.610 $\pm$ 0.009 & 0.534 $\pm$ 0.011 \\
Conformal risk filter & 0.611 $\pm$ 0.015 & 0.620 $\pm$ 0.020 & 0.569 $\pm$ 0.014 \\
Proposed branch mechanism & 0.682 $\pm$ 0.016 & 0.699 $\pm$ 0.024 & 0.648 $\pm$ 0.019 \\
Oracle branch upper bound & 0.754 $\pm$ 0.015 & 0.768 $\pm$ 0.020 & 0.735 $\pm$ 0.018 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.663 $\pm$ 0.012 & all components \\
Minus branch atlas & 0.556 $\pm$ 0.020 & removes explicit unrealized futures \\
Minus tail risk & 0.574 $\pm$ 0.019 & uses mean predicted value only \\
Minus diagnostic probe & 0.596 $\pm$ 0.010 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.609 $\pm$ 0.019 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/74.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Data Engine Failure Stratification is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
